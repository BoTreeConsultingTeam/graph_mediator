...
#        include ActiveSupport::Callbacks
  ...
  module AliasExtension #:nodoc:
    ...
    def _alias_method_chain_ensuring_inheritability(target, feature, &block)
      ...
      #
      # Comments: "_method_defined" method uses "instance_methods" and
      # "private_instance_methods" to collect methods.However the resulting
      # methods array holds method names of type Symbol(when using Ruby 2+),
      # instead of String.Thus when "_method_defined(with_method)" is invoked
      # it returns false because "with_method" variable holds a value of type
      # String which was causing NameError exception.
      # Fixed the problem by passing a "with_method" variable value as a Symbol.
      #
      # Author: Jignesh Gohel
      target_method_exists = _method_defined(with_method.to_sym)
